...
LABEL maintainer="Walter Doekes <wjdoekes+spandsp@osso.nl>"

ARG osdistro=debian
ARG osdistshort=deb
ARG oscodename=stretch
ARG upname=spandsp
ARG upversion=0.0.6+dfsg+snap20180108
ARG debepoch=
...
RUN upversion_uscore=$(echo $upversion | sed -e 's/~/_/g') && \
    ...
    curl -L --fail "http://junk.devs.nu/s/spandsp_0.0.6+dfsg+snap20180108.orig.tar.gz" \
    >/build/${upname}_${upversion}.orig.tar.gz && \
    echo "df59386730119e93fba51440ee5689ae  /build/${upname}_${upversion}.orig.tar.gz" | \
    md5sum -c
